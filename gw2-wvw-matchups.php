...
if (version_compare(PHP_VERSION, '5.3.0', '<') ) {
	...
	wp_die(sprintf(__('Your PHP version is too old, please upgrade to a newer version. Your version is %s, Guild Wars 2 - WvW Matchups requires %s. Remove the plugin from WordPress plugins directory with FTP client.', 'gw2-wvw-matchups'), phpversion(), '5.3.0 +'));
}
...
class gw2_wvw_matchups_widget extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		$lang = $instance['lang']; if (!$lang) { $lang = "en"; } 
		...
		// World Names API works again (v2.1)
		$world_names = $gw2->getWorlds(false, $lang);
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('skin'); ?>"><?php _e('Skin:', 'gw2-wvw-matchups'); ?></label>
			<select name="<?php echo $this->get_field_name('skin'); ?>" id="<?php echo $this->get_field_id('skin'); ?>" class="widefat">
			<?php		
				$skins = array(
					"default" => array("skin_id" => "default", "skin_name" => __('Default', 'gw2-wvw-matchups')),
					"vivid-colors" => array("skin_id" => "vivid-colors", "skin_name" => __('Vivid Colors', 'gw2-wvw-matchups'))
				);
		...
		$cachedir = dirname(__FILE__).'/cache'; if (substr(decoct(fileperms($cachedir)),2) != '777') : ?>			
			<p style="background: red; color: #fff; padding: 5px; margin-top: 20px;"><?php echo sprintf(__('<b>IMPORTANT:</b> To reduce load time you should look at the permissions on the folder "cache" in the plugin directory %s. You have to give write permissions (777).', 'gw2-wvw-matchups'), '"/wp-content/plugins/guild-wars-2-wvw-matchups/cache"'); ?></p>
		<?php endif; ?>
		<?php
	}
